...
RSpec.describe VideoProcessingController, type: :controller do
  context "valid audio file" do
    let(:audio_file) do
      File.new Rails.root.join("spec", "fixtures", "call_everybody.wav")
    end
    ...
    describe "#transcribe" do
      it "accepts text file and returns keywords" do
        get :transcribe, { audio_file: audio_file }

        expect(response.status).to eq(200)
        expect(response.content_type).to eq("application/json")
        expect(JSON.parse(response.body)["results"]['transcription']).to eq('this is a test')
      end
    end
